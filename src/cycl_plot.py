...
def draw_figure(hist_data, min, max, avg):
    ...
    for cpu_no in hist_data.keys():
        ...
        for index in range(0, len(x) - 1):
            ...
            variant += (x[index] - avg[cpu_no-1]) * \
                (x[index] - avg[cpu_no-1]) * y[index]
            samples += y[index]

        variant = variant/samples
        # print(variant, samples)
        stddev = variant**(.5)
        plt.plot(x, y, "-"+color_table[cpu_no - 1]+'*', label="cpu[%s]:avg=%d,min=%d,max=%d,std=%f" %
                 (cpu_no, avg[cpu_no-1], min[cpu_no-1], max[cpu_no-1], stddev))
    ...
    plt.xlabel("Latency (us): avg=%d, min=%d, max=%d" %
               (lat_avg/cpu_count, lat_min, lat_max))
